"""
engine.py — Moteur de recherche TF-IDF avec similarité cosinus (base SQLite).
"""
...
class SearchEngine:
    ...
    def search(
        ...
        # Normalisation des scores pour afficher 50%-95%
        normalized_scores = self._normalize_scores(final_scores)

        paginated_scores = normalized_scores[offset:offset + limit]

        results = []
        for chunk_id, score in paginated_scores:
    ...
    def _normalize_scores(self, raw_scores: list, min_pct: float = 50.0, max_pct: float = 95.0) -> list:
        """Normalise les scores bruts en pourcentages (50%-95%).

        Le meilleur résultat obtient max_pct, le plus faible min_pct.
        Les autres sont interpolés proportionnellement.
        """
        if not raw_scores:
            return []

        if len(raw_scores) == 1:
            return [(raw_scores[0][0], max_pct)]

        # Extraire les scores bruts
        chunk_ids = [s[0] for s in raw_scores]
        raw_values = [s[1] for s in raw_scores]
        ...
        if max_raw == min_raw:
            return [(cid, max_pct) for cid in chunk_ids]

        normalized = []
        for chunk_id, raw_val in raw_scores:
            # Interpolation linéaire entre min_pct et max_pct
            normalized_val = min_pct + (raw_val - min_raw) / (max_raw - min_raw) * (max_pct - min_pct)
            normalized.append((chunk_id, round(normalized_val, 1)))
