...
SCHEMA_SQL = """
-- ============================================================
-- ATLAS - Schéma de base de données
...
-- Table des passages chunkés pour l'indexation TF-IDF
...
-- Table de l'index TF-IDF (inverted index)
...
-- Table du vocabulaire avec fréquence documentaire
...
-- Table des statistiques globales d'indexation
...
def create_db(db_path: Optional[Path] = None) -> sqlite3.Connection:
    """Crée la base de données Atlas et retourne une connexion.

    Args:
        db_path: Chemin vers le fichier .db. Si None, utilise DATABASE_PATH.

    Returns:
        connexion sqlite3 avec row_factory et pragmas configurés.
    """
...
def insert_document(
    ...
    pages: int = 1,
    # Paramettes en anglais pour compatibilite
    title: str = "",
    content: str = "",
    file_path: str = "",
    author: str = "",
    ...
    """Insere un document historique dans la base.

    Args:
        conn:            Connexion SQLite active.
        titre/title:     Titre du document.
        contenu/content: Texte complet du document.
        fichier/path:    Chemin du fichier source (unique).
        auteur/author:   Auteur du document.
        source:          Source d'origine.
        periode/period:  Periode historique (ex: 'XIXe siecle').
        region:          Region geographique.
        langue/language: Langue du document.
        type_doc/type:   Type de document (Livre, Article, Archive).
        date_publication: Date de publication.
        pages/total_pages: Nombre total de pages.
        created_at:      Date et heure de création (format JJ/MM/AAAA HH:MM:SS). Si None, utilise l'heure locale actuelle.

    Returns:
        ID du document insere.
    """
    # Utilise les paramettes francais si fournis, sinon anglais
...
def insert_chunks(
    ...
    """Insère les passages (chunks) d'un document.

    Args:
        conn:        Connexion SQLite active.
        document_id: ID du document parent.
        chunks:      Liste de dicts {'content': str, 'token_count': int}.
...
def insert_tfidf_entries(
    ...
    """Insère les valeurs TF-IDF dans l'inverted index.

    Args:
        conn:     Connexion SQLite active.
        entries:  Liste de dicts {'chunk_id': int, 'term': str, 'tfidf_value': float}.
    """
    conn.executemany(
        """
...
def insert_vocabulary(
    ...
    """Insère ou met à jour un terme dans le vocabulaire.

    Args:
        conn:                Connexion SQLite active.
        term:                Terme tokenisé.
        document_frequency:  Fréquence documentaire à ajouter.
    """
    conn.execute(
        """
...
def get_documents(
    ...
    Args:
        ...
        period:  Filtre par période historique.
        region:  Filtre par région.
        author:  Filtre par auteur.

    Returns:
        Liste de Row sqlite3 contenant les colonnes du document.
    """
...
def get_document_by_id(conn: sqlite3.Connection, doc_id: int) -> Optional[sqlite3.Row]:
    ...
    Returns:
        Row sqlite3 ou None si introuvable.
    """
    return conn.execute(
...
def get_chunks_by_document(conn: sqlite3.Connection, document_id: int) -> List[sqlite3.Row]:
    ...
    Returns:
        Liste de Row triés par chunk_index.
    """
    return conn.execute(
...
def save_search(
    ...
    Args:
        conn:          Connexion SQLite active.
        query:         Requête de recherche saisie par l'utilisateur.
        result_count:  Nombre de résultats retournés.
        search_time_ms: Temps d'exécution en millisecondes.
        filters:       Filtres appliqués au format JSON string.
        searched_at:   Date et heure de la recherche (format JJ/MM/AAAA HH:MM:SS). Si None, utilise l'heure locale actuelle.

    Returns:
        ID de l'entrée historique créée.
    """
...
def get_search_history(
    ...
    Returns:
        Liste de Row triées par date décroissante.
    """
    return conn.execute(
...
def get_vocabulary(
    ...
    """Récupère le vocabulaire indexé.

    Args:
        conn:    Connexion SQLite active.
        limit:   Nombre max de termes.
        min_df:  Fréquence documentaire minimale.

    Returns:
        Liste de Row avec term et document_frequency.
    """
    return conn.execute(
...
def get_index_stats(conn: sqlite3.Connection) -> Optional[sqlite3.Row]:
    """Récupère les statistiques globales d'indexation.

    Args:
        conn: Connexion SQLite active.

    Returns:
        Row avec total_documents, total_chunks, total_terms, last_indexed_at.
    """
    return conn.execute("SELECT * FROM index_stats WHERE id = 1").fetchone()


def update_index_stats(conn: sqlite3.Connection) -> None:
    """Recalcule et met à jour les statistiques d'indexation.
...
def delete_document(conn: sqlite3.Connection, doc_id: int) -> bool:
    """Supprime un document et ses chunks associés (CASCADE).
...
def reset_index(conn: sqlite3.Connection) -> None:
    """Réinitialise complètement l'index de recherche.
...
def search_documents(
    ...
    """Recherche par termes dans l'inverted index TF-IDF.

    Args:
        conn:    Connexion SQLite active.
        terms:   Liste de termes tokenisés/stemmés.
        limit:   Nombre max de résultats.
        offset:  Décalage pour la pagination.

    Returns:
        Liste de dicts avec chunk_id, document_id, title, snippet, score.
    """
